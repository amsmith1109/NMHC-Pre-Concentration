...
class CNi:
        ...
        while True:
            ...
            if abs(time.time() - start_time) > 5:
                if __name__ == '__main__':
                    print(f'Unable to connect with omega device on port: {com}.')
    ...
    # 0x17 - 0x19
    def pid(self, p=None, i=None, d=None):
        ''' pid accesses the PID values for output #1
        (p) = dead band (Address 17)
        (i) = Reset (Address 18)
        (d) = Rate (Address 19)
        
        An empty call `pid()` will grab the current PID values from the controller.
        '''
        pid = [p, i, d]
        reset_flag = False
        max_val = [39321, 14745, 14745]
        for n, value in enumerate(pid):
            try:
                if value is None:
                    result = bc.msg2dec(self.echo(f'R{17+n}'))
                    pid[n] = result
                elif value > max_val[n] or value < 0:
                    raise ValueError(f'{"pid"[n]} value must be between 0 - {max_val[n]}')
                else:
                    code = hex(value)[2:]
                    while len(code) < 4:
                        code = '0' + code
                    self.echo(f'W{17+n}{code}')
                    reset_flag = True
