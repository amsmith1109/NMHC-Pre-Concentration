...
class ADS1115:
    _CONV = [1.875e-4, 1.25e-4, 6.25e-5, 3.125e-5, 1.5625e-5, 7.8125e-6]
    ...
    def single(self, channel=None):
        if channel is None:
            channel = [0, 1, 2, 3]
        if isinstance(channel, list):
            VAL = []
            for n, i in enumerate(channel):
                txt = 'Channel ' + str(i) + ': '
                VAL.append(self.single(i))
                txt = txt + str(VAL[-1]) + ' V'
                print(txt)
        if isinstance(channel, int):
            VAL = self.read(channel)*self._CONV[self.gain]
        return VAL

class ADS1015(ADS1115):
    _CONV = [3.0e-3, 2.0e-3, 1.0e-3, 5.0e-4, 2.5e-4, 1.25e-4]
    ...
    def single(self, channel):
        VAL = self.read(channel)
        return VAL*self._CONV[self.gain]